...
function s_doPlugins(s) {
	...
	if (s.pageName) {
        ...
        if( ppv && typeof ppv=='object' && ppv[0] == s.prop43 ) { //if ppv array returned and prev page id matches prev page name
          s.prop44 = ppv[1] + '|' + ppv[2]; //prop44: prev page max and initial % viewed, delimited by "|". (Use classifications to separate values)
          s.prop45 = ppv[4] + 'x' + ppv[5]; //prop45: viewport width x height in CSS pixels
          s.prop46 = ppv[1]; //prop46: prev page max.
          s.prop47 = ppv[2]; //prop47: initial % viewed.
        }
	} 
}
